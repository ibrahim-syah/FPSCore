...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MyGameplayAbility_HitscanWeapon)
...
UMyGameplayAbility_HitscanWeapon::UMyGameplayAbility_HitscanWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UFPSRangedWeaponInstance* UMyGameplayAbility_HitscanWeapon::GetWeaponInstance() const
{
	return Cast<UFPSRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UMyGameplayAbility_HitscanWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		if (GetWeaponInstance() == nullptr)
		{
			UE_LOG(LogLyraAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				...
				*UFPSRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UMyGameplayAbility_HitscanWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UMyGameplayAbility_HitscanWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UMyGameplayAbility_HitscanWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Lyra_TraceChannel_Weapon;
}

FHitResult UMyGameplayAbility_HitscanWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UMyGameplayAbility_HitscanWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UMyGameplayAbility_HitscanWeapon::GetTargetingTransform(APawn* SourcePawn, ELyraAbilityTargetingSource Source) const
{
	check(SourcePawn);
...
FHitResult UMyGameplayAbility_HitscanWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
...
void UMyGameplayAbility_HitscanWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());
...
void UMyGameplayAbility_HitscanWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UFPSRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
...
void UMyGameplayAbility_HitscanWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UMyGameplayAbility_HitscanWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UMyGameplayAbility_HitscanWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
...
void UMyGameplayAbility_HitscanWeapon::StartRangedWeaponTargeting()
{
